...
export default function EngineerListScreen() {
    ...
    useEffect(() => {
        const q = query(collection(db, 'user'), where('role', '==', 'Engineer'));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const engineerData = [];
            querySnapshot.forEach((doc) => engineerData.push({ id: doc.id, ...doc.data() }));
    ...
    const renderEngineerItem = ({ item }) => (
        <TouchableOpacity
            ...
            onPress={() => router.push({ pathname: '/analytic-detail', params: { name: item.name } })}
        >
    ...
    return (
        // 🚀 3. Changed SafeAreaView to standard View with fixed top padding
        <View style={[styles.container, { paddingTop: 10 }]}>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<Text style={styles.emptyText}>No engineers found in this category.</Text>}
                />
            )}
        </View>
    );
...
// Keep all the exact same styles below here...
const styles = StyleSheet.create({
    ...
    searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F1F4F9', borderRadius: 25, paddingHorizontal: 20, height: 50, borderWidth: 1, borderColor: '#E0E0E0' },
    searchInput: { flex: 1, fontSize: 16, color: '#333' },
    tabBar: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#E0E0E0', marginBottom: 10 },
    ...
    engineerItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 0.5, borderBottomColor: '#F0F0F0' },
    avatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#FDF2E9', justifyContent: 'center', alignItems: 'center', marginRight: 15 },
    avatarText: { color: '#D35400', fontWeight: 'bold', fontSize: 16 },
    engineerName: { fontSize: 16, color: '#333', fontWeight: '500' },
    skillText: { fontSize: 12, color: '#777' },
